...
//       const { data } = await axios.post(`https://localhost:7116/api/Feedback/AddGeneralFeedback?studentId=${userId}`, formData,{ headers: { Authorization: `Bearer ${userToken}` } });
//         console.log(data);
//       formik.resetForm();
//       setOpen(false);
...
// const textAraeInput = inputs.map((input,index)=>
//     <TextArea
//       type={input.type}
//       id={input.id}
//       name={input.name}
...
export default function AddGeneralFeedback({setOpen}) {
    ...
    const textAreaInput = inputs.map((input, index) =>
        <TextArea
            type={input.type}
            id={input.id}
            name={input.name}
